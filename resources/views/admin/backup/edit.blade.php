...
<div class="card" style="max-width:800px;">
    <div class="card-header">
        <span><i class="fas fa-edit"></i>&ensp;Schedule Configuration</span>
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route('admin.backup.update', $backup) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-row" style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                <div class="form-group">
                    ...
                    <select id="backup_type" name="backup_type" class="form-control">
                        <option value="database" {{ old('backup_type', $backup->backup_type) === 'database' ? 'selected' : '' }}>
                            Database Only
                        </option>
                        <option value="database_files" {{ old('backup_type', $backup->backup_type) === 'database_files' ? 'selected' : '' }}>
                            Database + Files
                        </option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="frequency" name="frequency" class="form-control" onchange="toggleDayFields()">
                        <option value="daily" {{ old('frequency', $backup->frequency) === 'daily' ? 'selected' : '' }}>Daily</option>
                        <option value="weekly" {{ old('frequency', $backup->frequency) === 'weekly' ? 'selected' : '' }}>Weekly</option>
                        <option value="monthly" {{ old('frequency', $backup->frequency) === 'monthly' ? 'selected' : '' }}>Monthly</option>
                    </select>
                </div>
            ...
            <div class="form-row" style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px;">
                <div class="form-group" id="day_of_week_group">
                    ...
                    <select id="day_of_week" name="day_of_week" class="form-control">
                        ...
                        <option value="{{ $i }}" {{ old('day_of_week', $backup->day_of_week) == $i ? 'selected' : '' }}>
                            {{ $day }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group" id="day_of_month_group">
                    ...
                    <select id="day_of_month" name="day_of_month" class="form-control">
                        @for($d = 1; $d <= 28; $d++)
                        <option value="{{ $d }}" {{ old('day_of_month', $backup->day_of_month) == $d ? 'selected' : '' }}>
                            {{ $d }}
                        </option>
                        @endfor
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="time"
                           ...
                           name="time_of_day"
                           class="form-control"
                           value="{{ old('time_of_day', substr($backup->time_of_day, 0, 5)) }}"
                           required>
            ...
            <div class="form-row" style="display:grid; grid-template-columns:2fr 1fr; gap:16px;">
                <div class="form-group">
                    ...
                    <select id="destination_drive_id"
                            ...
                            required>
                        @foreach($drives as $drive)
                        <option value="{{ $drive->id }}" {{ old('destination_drive_id', $backup->destination_drive_id) == $drive->id ? 'selected' : '' }}>
                            {{ $drive->name }}
                ...
                <div class="form-group">
                    ...
                    <input type="number"
                           ...
                           class="form-control"
                           value="{{ old('retention_count', $backup->retention_count) }}"
                           min="1"
                           max="100">
            ...
            <div class="form-group">
                <label style="display:flex; align-items:center; gap:8px; cursor:pointer;">
                    <input type="checkbox"
                           name="is_active"
                           value="1"
                           {{ old('is_active', $backup->is_active) ? 'checked' : '' }}
                           style="cursor:pointer;">
                    <span>
                ...
                <div class="form-help" style="margin-left:28px;">
                    Uncheck to pause this backup schedule
                </div>
            </div>

            {{-- Schedule Info --}}
            @if($backup->next_run_at)
            <div style="padding:12px 16px; background:var(--info-light); border:1px solid var(--info-border); border-radius:var(--radius-md); margin-top:16px;">
                <div style="font-size:12px; font-weight:600; color:var(--info-text); margin-bottom:4px;">
                    <i class="fas fa-clock"></i> Next Scheduled Run
                </div>
                <div style="font-size:13px; color:var(--info-text);">
                    {{ $backup->next_run_at->format('l, F j, Y \a\t g:i A') }}
                    <span class="text-muted">({{ $backup->next_run_at->diffForHumans() }})</span>
                </div>
            </div>
            @endif
